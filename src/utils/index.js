...
export const addScriptsInRootPackageJson = (rootDir) => {
  const scripts = {
    ...
    "xcode": "cd apps/mobile && ./ios-only.sh 'xed -b ios'",
    "touch-xcode": "cd apps/mobile && ./ios-only.sh 'touch ios/.xcode.env'",
    "setup-fastlane": "cd apps/mobile && rbenv local && bundle install && bundle update",
    "deploy:mobile": "cd apps/mobile && ./deploy.sh",
    "ios-certificates": "cd apps/mobile && ./ios-only.sh 'rbenv local && bundle install && bundle update && bundle exec fastlane ios certificates --env development'",
    "pod-install": "npx nx pod-install mobile",
    "pod-install:force": "npx nx pod-install:force mobile",
    "list:ios-configurations": "cd apps/mobile && ./ios-only.sh 'cd ios && xcodebuild -list'",
    "list:ios-devices": "cd apps/mobile && ./ios-only.sh 'xcrun xctrace list devices'",
